...
path = '/teamspace/studios/this_studio/.cache/kagglehub/datasets/kadircandrisolu/birdclef25-mel-spectrograms/versions/1'
@dataclass
class CFG:
    ...
    debug: bool = False
    use_wandb: bool = True
    # subset_size: Union[bool, Tuple[int, int]] = False  # (4000, 1000)
    subset_size: Tuple = (4000, 1000)
    apex: bool = False
    print_freq: int = 100
    num_workers: int = 0
    ...
    train_datadir: str = 'data/train_audio'
    train_csv: str = '/teamspace/uploads/train.csv'
    ...
    taxonomy_csv: str = '/teamspace/uploads/taxonomy.csv'

    spectrogram_npy: str = path + '/birdclef2025_melspec_5sec_256_256.npy'
    ...
    epochs: int = 10
    batch_size: int = 32
    criterion: str = 'focal_loss'  # 'BCEWithLogitsLoss'

    n_fold: int = 5
    selected_folds: List[int] = (0,)
...
def process_audio_file(audio_path, cfg):
    ...
    try:
        ...
        max_start = max(0, len(audio_data) - target_samples)
        ...
        if len(segment_audio) < target_samples:
            segment_audio = np.pad(segment_audio,
                                   (0, target_samples - len(segment_audio)),
                                   mode='constant')

        mel_spec = audio2melspec(segment_audio, cfg)

        if mel_spec.shape != cfg.TARGET_SHAPE:
            mel_spec = cv2.resize(mel_spec, cfg.TARGET_SHAPE,
...
def generate_spectrograms(df, cfg):
    ...
    print(
        f"Successfully processed {len(all_bird_data)} files out of {len(df)}")
...
class BirdCLEFDatasetFromNPY(Dataset):
    def __init__(self, df, cfg, spectrograms=None, mode="train"):
        ...
        self.label_to_idx = {label: idx for idx,
                             label in enumerate(self.species_ids)}

        if 'filepath' not in self.df.columns:
            self.df['filepath'] = self.cfg.train_datadir + \
                '/' + self.df.filename
        ...
        if self.spectrograms:
            found_samples = sum(
                1 for name in sample_names if name in self.spectrograms)
            print(
                f"Found {found_samples} matching spectrograms for {mode} dataset out of {len(self.df)} samples")

        if cfg.debug:
            self.df = self.df.sample(
                min(1000, len(self.df)), random_state=cfg.seed).reset_index(drop=True)
...
class BirdCLEFModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.mixup_enabled = hasattr(
            cfg, 'mixup_alpha') and cfg.mixup_alpha > 0
...
def train_one_epoch(model, loader, optimizer, criterion, device, scheduler=None):
    ...
    accuracy = calculate_accuracy(all_targets, all_outputs)
...
def run_training(df, cfg):
    ...
    if cfg.LOAD_DATA:
        ...
        try:
            spectrograms = np.load(cfg.spectrogram_npy,
    ...
    if not cfg.LOAD_DATA:
        ...
        if 'samplename' not in df.columns:
            df['samplename'] = df.filename.map(lambda x: x.split(
                '/')[0] + '-' + x.split('/')[-1].split('.')[0])
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['primary_label'])):
        ...
        if cfg.subset_size:
            train_dataset = Subset(train_dataset, range(cfg.subset_size[0]))
            val_dataset = Subset(val_dataset, range(cfg.subset_size[1]))
        ...
        for epoch in range(cfg.epochs):
            ...
            train_loss, train_auc, train_acc = train_one_epoch(
                ...
                criterion,
                cfg.device,
                scheduler if isinstance(
                    scheduler, lr_scheduler.OneCycleLR) else None
            )
